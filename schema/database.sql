...
-- =====================================================
CREATE OR REPLACE
ALGORITHM = UNDEFINED VIEW `svp`.`vw_CurrentFloorInventory` AS
select
    `svp`.`InventoryScans`.`Barcode` AS `Barcode`,
    `svp`.`InventoryScans`.`ProductID` AS `ProductID`,
    `svp`.`InventoryScans`.`ProductName` AS `ProductName`,
    sum(`svp`.`InventoryScans`.`Quantity`) AS `QuantityOnFloor`,
    min(`svp`.`InventoryScans`.`ScanDateTime`) AS `FirstScanned`,
    max(`svp`.`InventoryScans`.`ScanDateTime`) AS `LastScanned`
from
    `svp`.`InventoryScans`
where
    `svp`.`InventoryScans`.`Status` = 'OnFloor'
group by
    `svp`.`InventoryScans`.`Barcode`,
    `svp`.`InventoryScans`.`ProductID`,
    `svp`.`InventoryScans`.`ProductName`;
